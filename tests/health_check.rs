...
async fn subscribe_returns_a_400_when_data_is_invalid() {
    ...
    let client = reqwest::Client::new();
    let test_cases = vec![
        ("email=ursula_le_guingmail.com", "no @ will not pass regex"),
        ("email=ursula_le_guin%40gmail", "no . will not pass regex"),
    ];
    for (invalid_body, error_message) in test_cases {
        // Act
        let response = client
...
async fn subscribe_returns_a_200_when_data_is_valid() {
    ...
    let client = reqwest::Client::new();
    let test_cases = vec![(
        "email=ursula_le_guin%40gmail.com&name=leguin",
        "email is valid",
    )];
    for (valid_body, message) in test_cases {
        // Act
        let response = client
            .post(&format!("{}/subscriptions", &app.address))
            .header("Content-Type", "application/x-www-form-urlencoded")
